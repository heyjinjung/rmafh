...
## Changelog
- 2025-12-25 v1.1: 업로드 검증 규칙/에러 코드 명시, UTF-8/BOM 안내, 불리언/날짜 파싱 범위 표준화
...
### 1.2 업로드 검증 규칙 (v1.1)
- 인코딩: UTF-8 권장, BOM 허용
- 최대 행 수: 10,000 (초과 시 `TOO_MANY_ROWS`)
- 필수 컬럼: `external_user_id` 누락 시 해당 행 스킵, 전부 누락 시 `EMPTY_ROWS`
- 중복 처리: 동일 `external_user_id`는 첫 번째 행만 사용, 나머지는 스킵
- 불리언 파싱: `true/false/1/0/yes/no` (대소문자 무시)
- 숫자 파싱: 쉼표 제거 후 정수 변환, 음수는 0으로 보정
- 날짜 파싱: ISO 8601 또는 `YYYY-MM-DD` 지원, 실패 시 NULL 저장 (에러 아님)
- 알 수 없는 컬럼: 무시하고 진행
- CSV 형식 오류: 빈 파일/헤더만 존재 시 `EMPTY_ROWS`

### 1.1 필수 컬럼 (한글)

| 한글 컬럼명 | 영문 컬럼명 (호환) | 타입 | 필수 | 설명 | 예시 |
|------------|-------------------|------|------|------|------|
| `아이디` | `external_user_id` | 문자열 | ✅ 필수 | 외부 유저 식별자 (고유값) | `user_hong123`, `external_12345` |
| `닉네임` | `nickname` | 문자열 | ⚠️ 권장 | 유저 닉네임 | `홍길동`, `casino_pro` |
| `입금액` | `deposit_total` | 숫자 | ⚠️ 권장 | 누적 입금액 (원 단위) | `1000000`, `500000` |
| `텔레그램` | `telegram_ok` | 불리언 | ⚠️ 권장 | 텔레그램 인증 완료 여부 | `true`, `false`, `1`, `0` |
| `가입일` | `joined_at` | 날짜/문자열 | ❌ 선택 | 가입일 (YYYY-MM-DD 또는 ISO) | `2024-12-01`, `2024-12-01T10:30:00Z` |
| `입금일` | `last_deposit_at` | 날짜/문자열 | ❌ 선택 | 마지막 입금 시각 (ISO 8601) | `2024-12-22T14:30:00Z` |
| `리뷰` | `review_ok` | 불리언 | ❌ 선택 | 리뷰 작성 완료 여부 | `true`, `false` |

**💡 중요**: 한글 컬럼명과 영문 컬럼명 모두 사용 가능합니다. 혼용도 가능합니다.
...
```csv
아이디,닉네임,deposit_total,텔레그램,joined_at
user_hong,홍길동,1000000,true,2024-12-01
user_kim,김철수,500000,false,2024-12-10
```
