...
## 3.2 변경 반영 노트 (v3.0)
- 플래티넘 조건이 "출석 3 AND 단일 50,000"에서 "골드 해금 + 누적 입금 200,000원 + 누적 입금 3회"로 변경됨
- 다이아 조건이 "누적 500,000"에서 "플래티넘 해금 + 누적 충전 2,000,000원 + CC카지노 출석부 기준 2회"로 변경됨
- 다이아 마감기한이 가입 후 5일(120시간)로 변경됨

## 4. 주요 케이스 (발췌)
- 상태 전이: LOCKED→UNLOCKED→CLAIMED, 만료 시 EXPIRED
- 플래티넘 조건(v3): (골드 해금 필수) AND (누적 입금 200,000원) AND (누적 입금 3회) 만족/불만족/부분만족
- 다이아 조건(v3): (플래티넘 해금 필수) AND (누적 충전 2,000,000원) AND (CC카지노 출석부 기준 2회) 만족/불만족/부분만족
- 출석: 하루 1회, 중복 시 409
