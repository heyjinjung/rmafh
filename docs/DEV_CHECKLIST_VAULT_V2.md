...
- [x] 실제 비즈니스 로직/에러 처리 보완 (현재 단순화된 로직)
	...
	- [x] 에러 코드 체계(404/409/403) 운영 기준으로 정리 및 문서화(API_ERRORS_VAULT_V2.md 반영)
...
- [x] 어드민 레이아웃/스크롤 최종 검증 (모바일/태블릿/데스크탑에서 세로 확장 체감 확인)
- [x] 어드민 접근 제어(인증/권한) 적용
- [x] 어드민 작업 감사 로그(누가/언제/무엇을) 및 운영 가이드 정리

## 사용자 인증/로그인
- [x] 유저 로그인 페이지 (`/login`) - 닉네임 기반
- [x] 로그인 API (`/api/vault/login`) - external_user_id 생성/매핑
- [x] 로그인 후 메인 페이지 연동 (external_user_id 쿼리 파라미터)
- [x] localStorage 세션 관리
- [x] CSV 업로드 컬럼 가이드 문서화 (docs/CSV_UPLOAD_COLUMN_GUIDE.md)
