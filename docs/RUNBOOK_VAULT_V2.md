...
## 2. 배포 체크리스트
- (운영 권장) 마이그레이션 적용 (vault_status 등)
- (로컬/테스트) `app.main` 스타트업의 `_ensure_schema()`가 최소 스키마를 best-effort로 자동 보강
...
- deposit-hook 오류: (현재 미구현) 구현 시 tx_id 충돌/외부 결제 지연/idempotency 점검
