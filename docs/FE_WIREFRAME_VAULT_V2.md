...
## Changelog
- 2025-12-20 v0.2: 플래티넘 20,000 기준 반영, 유저 출석 CTA 제거(수령 중심 UX)로 정합화

## 2. 홈 상단: 금고 대시보드 섹션
- 현재 구현(2025-12-20): 3개 카드 UI + 카운트다운 + CTA(수령) + 완성 보너스 섹션까지 구현됨.
...
### 2.2 Platinum 카드
- 보상: 20,000원, 강조색(경고)
- 진행률 표시(기본): (1) 연속 0/3 (운영 업로드 기반) (2) 리뷰 키 0/1
- 카피: "20,000원이 소멸 예정" (붉은 경고)
- CTA: "회수하기" (UNLOCKED일 때만 활성)
...
- 메시지: "잠시만요! 그냥 나가시게요? 금고에 20,000원이 울고 있어요." (보관/소멸 프레이밍)
- 서브 카피: "티켓은 없지만 플래티넘 금고가 열리면 즉시 20,000원 회수"
- CTA: "플래티넘 해금하러 가기" (secondary: "나중에 다시 보기")

## 4. 알림/토스트 패턴
- 진행 갱신: "D-?일, 20,000원 보관 중" 토스트
