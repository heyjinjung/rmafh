...
어드민 페이지에서 바로 버튼/입력을 해가며 유저에 대한 모든것을 관리할 수 있게 
기능 추가 해줘 
...
- vault_expiry_extension_log: 만료 연장/부활 이력 감사. 어드민에서 만료 연장/부활 버튼 클릭 시 request_id 기반으로 중복 방지 기록 필수.
- notifications_queue: 만료 D-2/D-day, 출석 리마인드, 소셜프루프 등 알림 큐. 어드민이 “알림 보내기”/“토스트 재발송” 같은 액션을 큐잉할 때 사용.
...
- vault_expiry_extension_log: id, user_id, prev_expires_at, new_expires_at, reason(OPS|PROMO|REFERRAL|ADMIN), request_id(UNIQUE), shadow, metadata(JSON), created_at
- notifications_queue: id, user_id, type(EXPIRY_D2|EXPIRY_D0|ATTENDANCE_D2|SOCIAL_PROOF|REFERRAL_REVIVE), vault_type(GOLD|PLATINUM|DIAMOND), variant_id, dedup_key(UNIQUE), payload(JSON), scheduled_at, status(PENDING|SENT|FAILED|DLQ), retry_count, created_at
...
- POST /admin/users/{user_id}/vault/extend-expiry: 만료 연장/부활. body: {request_id, extend_hours, reason, shadow=false}. 로그는 vault_expiry_extension_log에 기록.
- POST /admin/users/{user_id}/vault/referral-revive: 추천 부활 1회용. body: {request_id, channel, invite_code}. UNIQUE(user_id, reason=REFERRAL) 보장.
...
- 행 단위 액션 버튼: 출석 +1, 입금 토글, 금고 상태 전환(UNLOCK/CLAIM/EXPIRE), 만료 연장(+시간), 추천 부활, 알림 보내기.
...
- 허용 전이
	...
	- EXPIRED → UNLOCKED (부활/만료 연장 시, 요청 시점 상태가 EXPIRED일 때만)
- 차단 전이
	- CLAIMED 이후 다른 상태로 회귀 금지 (오류 반환)
	- EXPIRED가 아닌데 연장/부활 요청 시: 만료만 +시간 연장, 상태는 유지
...
```
- POST /admin/users/{id}/vault/referral-revive
```json
...
- 상태 패널(선택 행 기준): 만료 연장(+시간 입력), 출석 +1, 입금 토글, 금고 상태 전환(UNLOCK/CLAIM/EXPIRE), 추천 부활(request_id 자동), 알림 보내기(variant 선택).
- 데이터 패칭: SWR or React Query로 /admin/users 목록, 액션 후 invalidate/refetch. 낙관적 업데이트는 멱등 실패 시 롤백.
- 멱등키 생성: FE에서 UUID v4 생성하여 연장/부활/상태변경 요청에 request_id로 첨부.
...
4) 추천 상태 되살리기 기능을 백/프론트/데이터에서 모두 삭제해줘 
