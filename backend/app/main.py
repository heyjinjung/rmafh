...
# Phase 2: Include routers (Í∏∞Ï°¥ ?îÎìú?¨Ïù∏?∏Ï? Í≤ΩÎ°ú ?ôÏùº ?†Ï?)
...
def _parse_bool(value) -> bool:
    ...
    if not s:
        return False
    return s in {"1", "true", "t", "yes", "y", "ok", "o", "Ïùë", "ÌôïÏù∏", "ÏôÑÎ£å"}
...
async def user_daily_import(body: DailyUserImportRequest, request: Request, response: Response, _auth: str = Depends(verify_admin_password)):
    """?ºÏùº ?ëÏ?/CSV ?ÖÎ°ú???∞Ïù¥?∞Î°ú ?¥ÏòÅ ?§ÎÉÖ??+ Ï°∞Í±¥??Í∞±Ïã†.

    ?ÖÎ†•(Í∂åÏû•): external_user_id, nickname, deposit_total(?ÑÏ†Å), joined_at, last_deposit_at, telegram_ok

    Î∞òÏòÅ:
    - user_identity: external_user_id ??user_id Îß§Ìïë ?ùÏÑ±/?¥Í≤∞
    - user_admin_snapshot: ?ÖÎ°ú?úÎêú ?¥ÏòÅ Ïª¨Îüº ?ÖÏÑú??
    - vault_status:
...
async def user_identity_bulk(body: UserIdentityBulkRequest):
    """?∏Î? ?ÑÏù¥??Î™©Î°ù??user_identity???ºÍ¥Ñ ?±Î°ù/?¥Í≤∞.

    - ?ÖÎ†•: external_user_ids (CSV ?ÖÎ°ú??Í≤∞Í≥º)
    - ?ôÏûë: ?ÜÏúºÎ©??ùÏÑ±, ?àÏúºÎ©?Í∑∏Î?Î°??¨Ïö© (Î©±Îì±)
    """
...
async def extend_expiry(body: ExtendExpiryRequest, request: Request, response: Response, _auth: str = Depends(verify_admin_password)):
    """?¥ÏòÅ/?ÑÎ°úÎ™®ÏÖò ÎßåÎ£å ?∞Ïû•. shadow=trueÎ©?ÎØ∏Ï†Å???ÑÎ¶¨Î∑?"""
    ...
    with db.get_conn() as conn:
        ...
        _log_admin_action(
            ...
            endpoint="/api/vault/extend-expiry",
            target_user_ids=target_ids[:1000],  # ÏµúÎ? 1000Í∞úÎßå ?Ä??(?òÌîåÎß?
            request_id=key,
...
async def notify(body: NotifyRequest, request: Request, response: Response, _auth: str = Depends(verify_admin_password)):
    ...
    with db.get_conn() as conn:
        ...
        for uid in resolved_user_ids or []:
            ...
            # Ïª§Ïä§?Ä Î©îÏãúÏßÄ ?êÎäî ?úÌîåÎ¶?Î©îÏãúÏßÄ ?¨Ïö©
            payload_dict = {
                "type": body.type,
        ...
        _log_admin_action(
            ...
            action="NOTIFY",
            endpoint="/api/vault/notify",
            target_user_ids=resolved_user_ids[:1000],  # ÏµúÎ? 1000Í∞úÎßå ?Ä??
            request_id=key,
...
async def list_notifications(
    ...
    for row in rows:
        ...
        item = {
            ...
            "payload": payload,
            # payload?êÏÑú Î©îÏãúÏßÄ ?ÑÎìú Ï∂îÏ∂ú
...
async def admin_bulk_update(body: AdminBulkUpdateRequest, request: Request, response: Response, _auth: str = Depends(verify_admin_password)):
    ...
    def _apply_updates_for_user(cur, user_id: int, now: datetime):
        ...
        # CLAIMED?Ä ?òÎèåÎ¶¨Ï? ?äÏùå (per single-user behavior)
